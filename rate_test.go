...
func TestRateLimit(t *testing.T) {
	limit := 5
	rq := miser.RateQuota{MaxRate: miser.PerSec(1), MaxBurst: limit - 1}
	start := time.Unix(0, 0)
	cases := []struct {
		now               time.Time
	...
	rl, err := miser.NewGCRARateLimiter(&st, rq)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRateLimitUpdateFailures(t *testing.T) {
	rq := miser.RateQuota{MaxRate: miser.PerSec(1), MaxBurst: 1}
	mst, err := memstore.New(0)
	if err != nil {
		t.Fatal(err)
	}
