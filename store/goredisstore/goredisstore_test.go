...
import (
	...
	"github.com/pantianying/miser/store/storetest"
)

const (
	redisTestDB     = 1
	redisTestPrefix = "miser-go-redis:"
)
...
func ExampleNew() {
	...
	// Setup store
	store, err := goredisstore.New(client, "miser:")
	if err != nil {
		log.Fatal(err)
	}

	// Setup quota
	quota := miser.RateQuota{MaxRate: miser.PerMin(20), MaxBurst: 5}

	// Then, use store and quota as arguments for NewGCRARateLimiter()
	miser.NewGCRARateLimiter(store, quota)
}
